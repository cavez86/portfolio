...
- The `--frozen-lockfile` flag ensures reproducible installs by using the exact versions in `bun.lock`

### 2. Build Limitations

**IMPORTANT:** The build command (`bun run build`) will FAIL in sandboxed environments because it attempts to fetch
Google Fonts (Geist) from `fonts.googleapis.com`. This is expected behavior and NOT a bug to fix.
...
- `bun type-check` - TypeScript type validation (NOTE: Shows errors in `.next/types/validator.ts` - these are
  pre-existing, generated by Next.js, and ignored via `typescript.ignoreBuildErrors: true`)
- `bun lint` - ESLint with auto-fix (fast, ~10 seconds)
- `bun format` - Prettier formatting (fast, ~5 seconds)
...
2. `bun lint` - ESLint checks
3. `bun type-check` - TypeScript compilation check
4. `bun format` - Prettier formatting check
...
- Runs `bun format` on all changed files
- Runs `bun lint` on .js, .jsx, .ts, .tsx files
...
bun type-check        # TypeScript type checking (no emit)
...
```bash
bun payload [command]  # Access Payload CLI (requires PAYLOAD_CONFIG_PATH env var)
```
...
**Expected behavior** in sandboxed environments. Use `bun type-check` instead to validate TypeScript.
...
The warning about ignored build scripts for `@vercel/speed-insights`, `sharp`, etc. may appear. This is normal and can
be ignored.

### 4. TypeScript Type Errors in .next Directory

The `bun type-check` command reports errors in `.next/types/validator.ts`. These are:
